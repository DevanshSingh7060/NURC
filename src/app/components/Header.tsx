...
export function Header() {
  ...
  return (
    <header
      role="banner"
      className={`sticky top-0 border-b border-border bg-card/95 backdrop-blur-sm transition-all duration-200 font-heading ${
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16 sm:h-20 relative">
        ...
        >
          {navLinks.map((link) =>
            link.hasDropdown ? (
              ...
              >
                <button className="flex items-center gap-1 px-3 py-2 rounded text-sm font-medium transition-colors cursor-pointer text-nurc-navy">
                  {link.label}
                  <ChevronDown size={14} />
                </button>
                {sectorsOpen && (
                  <div className="absolute top-full left-0 mt-1 w-52 bg-card border border-border rounded-lg shadow-lg py-1 z-50">
                    {solutionsLinks.map((s) => (
                      <Link
                        key={s.to}
                        to={s.to}
                        className="block px-4 py-2 text-sm hover:bg-muted transition-colors text-left font-medium text-nurc-navy"
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          >
            {mobileOpen ? (
              <X size={20} className="text-nurc-navy" />
            ) : (
      ...
      <>
        ...
        <div
          className={`fixed inset-0 lg:hidden bg-black/45 backdrop-blur-sm transition-opacity duration-300 w-screen h-screen z-[9998] ${
        ...
        <div
          className={`fixed top-0 bottom-0 right-0 w-[80vw] max-w-[380px] border-l border-border p-6 flex flex-col justify-between shadow-2xl transition-transform duration-300 ease-in-out transform lg:hidden h-screen z-[9999] bg-white overflow-y-auto font-heading ${
            mobileOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        >
          <div className="space-y-6 overflow-y-auto animate-none [scrollbar-width:thin]">
            {/* Drawer Logo header */}
            <div className="flex items-center justify-between border-b border-border pb-4">
              <Link to="/" className="flex items-center gap-2" onClick={() => setMobileOpen(false)}>
                <div className="w-7 h-7 rounded flex items-center justify-center text-white text-xs font-bold bg-nurc-navy font-display">
                  N
                </div>
                <div className="leading-none text-left">
                  <div className="font-bold text-sm text-nurc-navy">NURC</div>
                  <div className="font-semibold uppercase text-[8px] text-nurc-teal tracking-[0.12em]">
                    MediaNext
                  </div>
                </div>
              </Link>
            ...
            {/* Links drawer list */}
            <nav
              className="flex flex-col gap-1 text-sm font-bold text-left text-nurc-navy"
            ...
            >
              <Link
                ...
                onClick={() => setMobileOpen(false)}
              >
                <Home size={15} className="text-nurc-teal" />
                Home
              </Link>
              ...
              <div>
                <button
                  ...
                  className="w-full px-3 py-2 rounded-lg hover:bg-muted transition-colors flex items-center justify-between text-sm font-bold bg-transparent border-0 cursor-pointer text-left text-nurc-navy font-heading"
                >
                  <span className="flex items-center gap-2.5">
                    <Briefcase size={15} className="text-nurc-teal" />
                    Solutions
                  </span>
                  <ChevronDown
                    size={14}
                  ...
                  <div className="pl-8 flex flex-col gap-1.5 mt-1 border-l border-teal/20 ml-5">
                    {solutionsLinks.map((s) => (
                      <Link
                        key={s.to}
                        to={s.to}
                        className="px-3 py-1.5 rounded-lg hover:bg-muted transition-colors text-xs text-left font-semibold text-nurc-navy"
                        onClick={() => setMobileOpen(false)}
                      >
                        {s.label}
                      </Link>
              ...
              <Link
                ...
                onClick={() => setMobileOpen(false)}
              >
                <Settings size={15} className="text-nurc-teal" />
                Services
              </Link>
              ...
              <Link
                ...
                onClick={() => setMobileOpen(false)}
              >
                <FileText size={15} className="text-nurc-teal" />
                Resources
              </Link>
              ...
              <Link
                ...
                onClick={() => setMobileOpen(false)}
              >
                <User size={15} className="text-nurc-teal" />
                About
              </Link>
              ...
              <Link
                ...
                onClick={() => setMobileOpen(false)}
              >
                <Mail size={15} className="text-nurc-teal" />
                Contact
              </Link>
              ...
              {!currentUser ? (
                <div className="flex flex-col gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    <Link
                      to="/login"
                      className="py-2.5 rounded-lg border border-border text-center hover:bg-muted transition-colors text-xs flex items-center justify-center font-bold text-nurc-navy"
                      onClick={() => setMobileOpen(false)}
                    >
                      Login
                    </Link>
                    <Link
                      to="/signup"
                      className="py-2.5 rounded-lg text-white text-center hover:opacity-90 transition-opacity text-xs font-bold flex items-center justify-center bg-nurc-navy"
                      onClick={() => setMobileOpen(false)}
                    >
                      Sign Up
                    </Link>
                ...
                <div className="flex flex-col gap-1">
                  <Link
                    ...
                    onClick={() => setMobileOpen(false)}
                  >
                    <Layout size={15} className="text-nurc-teal" />
                    Dashboard
                  </Link>
                  ...
                  <Link
                    ...
                    onClick={() => setMobileOpen(false)}
                  >
                    <BookOpen size={15} className="text-nurc-teal" />
                    Archive
                  </Link>
          ...
          <div className="border-t border-border pt-4">
            {currentUser && (
              <div className="px-3 py-2 bg-[#F8F9FA] border rounded-xl text-xs text-muted-foreground leading-normal">
                <span className="font-bold block text-nurc-navy">{currentUser.fullName}</span>
