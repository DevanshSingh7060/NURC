...
const items: MarqueeItem[] = [
  ...
  { logoType: 'image', src: tvsLogo, name: 'TVS' },
  {
    ...
    name: 'Deutsche Bank',
    Logo: () => (
      <svg className="w-auto h-[54px] text-[#0018A8]" viewBox="0 0 160 50" fill="currentColor">
        <rect
          x="10"
          y="10"
          width="30"
  ...
  },
  {
    ...
    name: 'HDFC Life',
    Logo: () => (
      <svg className="w-auto h-[54px] text-[#0054A6]" viewBox="0 0 160 50">
        ...
        <rect
          ...
          strokeWidth="3.5"
        />
        <text x="50" y="26" fill="#000000" fontSize="15" fontWeight="bold" fontFamily="sans-serif">
          HDFC Life
        </text>
      </svg>
    ),
...
export function ClientMarquee({
  label = 'Trusted by India’s Leading Industry Leaders',
  compact = false,
}: Props) {
  ...
  return (
    <section className="bg-[#FFFFFF] select-none py-8">
      {label && !compact && (
        <h3 className="text-center font-heading text-[19px] font-semibold text-[#1F2937] mb-6 tracking-[-0.01em]">
          {label}
        </h3>
      )}
