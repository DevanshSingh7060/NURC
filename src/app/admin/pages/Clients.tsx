...
export function ClientsView() {
  ...
  const rows =
    statusFilter === 'all' ? data.clients : data.clients.filter((c) => c.status === statusFilter);

  const columns: Column<Client>[] = [
    {
      key: 'organization',
      header: 'Client',
      sortValue: (r) => r.organization.toLowerCase(),
      render: (r) => (
        <div>
          <p className="font-medium text-[var(--nurc-navy)]">{r.organization}</p>
          <p className="text-xs text-muted-foreground">
            {r.contactPerson} · {r.emailAddress}
          </p>
        </div>
      ),
    },
    {
      ...
      render: (r) => (
        <Select
          value={r.status}
          onValueChange={(v) => {
            update('clients', r.id, { status: v as Client['status'] });
    ...
    },
    {
      ...
      hideBelow: 'md',
      header: 'Phone',
      render: (r) => <span className="text-sm">{r.telephoneNumber}</span>,
    },
    {
      key: 'addDate',
      hideBelow: 'md',
      header: 'Add Date',
      sortValue: (r) => r.addDate,
      render: (r) => fmtDate(r.addDate),
    },
    {
      key: 'lastUpdated',
      hideBelow: 'lg',
      ...
      render: (r) => fmtDate(r.lastUpdated),
    },
    {
      key: 'actions',
      header: '',
      ...
      render: (r) => (
        <div className="flex justify-end gap-1">
          <Button
            ...
            onClick={() => toast.info('Send Newsletter is a demo action.')}
          >
          ...
          <Button
            ...
            className="size-8"
            title="Edit"
            onClick={() => toast.info('Edit is a demo action.')}
          >
          ...
          <Button
            ...
            title="Delete"
            onClick={() => {
              if (confirm(`Delete client "${r.organization}"?`)) {
                remove('clients', r.id);
  ...
  return (
    <div>
      <PageHeader
        title="Clients"
        description="Manage subscriber organisations and their status."
        actionLabel="Add Client"
        actionTo="/admin/clients/add"
      />
      <DataTable
        columns={columns}
        rows={rows}
